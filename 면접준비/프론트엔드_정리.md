...
Server side rendering를 이용하면 해결된다.
...
### 호이스팅
모든 함수, 변수를 var로 선언 될 때 스코프의 가장 상단으로 가는 것을 말한다.  
...
호이스팅
자바스크립트는 내부적으로 변수와 함수 선언부를 현재 스코프 제일 위쪽으로 호이스팅하기 때문에 이런일이 발생합니다.
...
참조투명성 : 함수는 반드시 동일입력을 동일결과에 매핑해야 합니다. 
...
0. 그리고 한개의 DATA 구조는 key - value 형태로 이루어져 있으며 _id라는 고유한 아이디를 가집니다. 
1. 스키마 없이 데이터모델을 구현하지 않은채 그냥 유동적으로 데이타를 삽입할 수 있습니다. 
* 하지만 스키마는 필요합니다. (아주 작은 테스트할 때만 스키마를 쓰지 않죠.)
* 스키마 : 데이타베이스를 구성하는 속성, 관계 등 데이타 값들이 갖는 type들을 명시 해놓은 것을 말합니다. 
2. min, max, aggreate, mapReduce 등 강력한 함수로 데이터를 뽑아내고 조합하는 등의 활동을 할 수 있습니다. data들을 모아서 초최대값을 뽑아내거나 최소값 등등의 행위를 할 수 있습니다.  
3. ReplicaSet을 이용해 이중화가 가능합니다. 
4. BJSON, Binary Json 형태로 저장합니다. json 형태가 아닙니다.
6. geoSpartial 인덱스를 써서 2Dimension 좌표  search에 활용이 가능합니다. 이것을 인덱싱이 자유롭다고 합니다.
WiredTiger라는 스토리지엔진의 저장방식은 LSM-Tree를 사용하는데 도큐먼트가 아무리 많아도 데이타를 저장하는데 동일한 수준의 성능을 보장한다는것을 알려주는데요. 저렇게 그림으로 정말 쉽게 알려줍니다. 옛날 버전의 엔진은 MMAPv1를 사용하는데 이 엔진보다는 성능이 낮습니다. 이 책을 보고 낮은 버전의 MongoDB를 높은 버전으로 올리는 것에 대한 확신을 가지게 되었고 실제 프로젝트에 적용하기로 결정했습니다. 
그 외 인상깊은 내용
샤딩
샤딩, 데이터베이스 서버를 확장하려면 데이터베이스의 데이터가 여러 서버로 분산될 수 있게 미리 응용 프로그램을 설계하고 개발하는 것을 말합니다. 
스토리지엔진
디스크에서 데이터를 어떻게 가져오고 어떻게 최적으로 저장할 것인지 결정하는 부분이 스토리지 엔진이다
정확한 표현
 - MongoDB는 SQL문법을 지원하지 않고 자바스크립트 기반의 명령을 이용하는데, 간단히 MongoDB 명령들은 기본적으로 JSON도큐먼트를 인자로 사용한다.  
 - 도큐먼트에서 _id 라는 필드가 자동으로 그 도큐먼트의 프라이머리 키로 선정된다. JSON 값을 문자열로 그대로 저장하는 것이 아니라 문자열 기반의 JSON텍스트를 BSON, Binary JSON형태로 저장한다. 
 - 인덱스의 기본 정렬은 항상 오름차순으로 구현돼 있지만 이 인덱스를 읽는 방향에 따라서 오름차순 또는 내림차순의 효과를 얻을 수 있으며 인덱스에서 각 필드의 정렬을 오름차순, 내림차순 으로 결정할 수 있다.  *대부분의 인덱스는 B-Tree를 적용 
...
1. 정렬 작업에 인덱스를 활용
2. 메모리 공간 더 크게
3. find명령어 대신 aggregate명령을 사용하고 allowDiskUse옵션을 true로 설정하는 것이다. 
### Node.js 
